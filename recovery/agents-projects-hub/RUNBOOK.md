...
Maintained for Hub `0.7.0`, state schema `33`. This capsule is a bounded local
...
SHA, schema `33/33`, and `deployment_revision.status=converged`. A live process
...
- Schema/integrity failure: stop database users and preserve evidence before a
  reviewed restore. Ordinary runtime rollback retains schema-32 state.
