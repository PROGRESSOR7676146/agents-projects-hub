...
### Содержательный итоговый отчёт turn (калибровка)
...
ответом без отчёта. Первые примеры агент создаёт сам, а оператор отмечает,
когда документ лишний; по этим примерам уточняется правило. Документ дополняет
обычный ответ и представляет результат в доступном виде:
...
Первый формат — Markdown через существующий per-turn artifact delivery.
Автоматический renderer и графические оформления остаются в backlog до накопления
примеров и проверки пользы. Генератор обязан соблюдать правила deliberate
