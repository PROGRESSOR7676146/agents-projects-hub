...
Статус: контракт реализован, офлайн-проверки добавлены; живая приёмка отдельно.
Актуальные решения и ограничения: [ADR 0022](../decisions/0022-explicit-codex-session-adoption.md).
...
- Сохранённая Codex-сессия по-прежнему не может быть принята прямо в
  lane-bound topic: её immutable origin проверяется относительно базового
  зарегистрированного project root. После отдельной локальной привязки свободной
  темы worker может выполнять новые обычные provider turns в проверенном lane;
  это не меняет origin уже принятой сессии.
