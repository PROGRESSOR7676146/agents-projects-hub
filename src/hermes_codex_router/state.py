...
class HubState:
    ...
    def _session(row: sqlite3.Row) -> SessionRecord:
        return SessionsStateFacade.record(row)
    ...
    def _insert_incoming_materials(
        ...
        *,
        ...
        self._incoming_material_state.insert(
            job_id=job_id,
            topic_id=topic_id,
            chat_id=chat_id,
    ...
    def _attach_forwarded_materials(
        ...
        *,
        ...
        self._incoming_material_state.attach_forwarded(
            job_id=job_id,
            topic_id=topic_id,
            agent_id=agent_id,
            session_id=session_id,
    ...
    def bind_provider_session(
        ...
        return self._sessions_state.bind_provider_session(
            session_id, provider_session_id, terminal_name
        )
    ...
    def _require_writer_transfer_snapshot(self, snapshot: WriterTransferSnapshot) -> None:
        ...
        self._sessions_state._require_writer_transfer_snapshot(snapshot)
    ...
    def set_writer_mode(
        ...
        *,
        ...
        return self._sessions_state.set_writer_mode(
            session_id,
            writer_mode,
    ...
    def return_codex_local_writer(
        ...
        *,
        ...
        return self._sessions_state.return_codex_local_writer(
            chat_id=chat_id,
            message_id=message_id,
            topic_id=topic_id,
            session_id=session_id,
    ...
    def _session_origin_exists(self, session_id: str) -> bool:
        from .session_adoption_state import CodexSessionOrigins

        return CodexSessionOrigins(self).get(session_id) is not None

    def _activate_session_origin(self, session_id: str, message_id: int, topic_id: int) -> None:
        from .session_adoption_state import CodexSessionOrigins
    ...
    def set_context_remaining(self, session_id: str, percent: float | None) -> SessionRecord:
        return self._sessions_state.set_context_remaining(session_id, percent)

    def telegram_contract_version(self, session_id: str) -> int:
        return self._sessions_state.telegram_contract_version(session_id)
    ...
    def telegram_contract_provenance(
        ...
        return self._sessions_state.telegram_contract_provenance(limit=limit)

    def acknowledge_telegram_contract(self, session_id: str, version: int) -> None:
        self._sessions_state.acknowledge_telegram_contract(session_id, version)
    ...
    def incoming_materials_for_job(self, job_id: str) -> tuple[IncomingMaterialRecord, ...]:
        return self._incoming_material_state.for_job(job_id)

    def pending_incoming_materials(self, topic_id: int) -> tuple[IncomingMaterialRecord, ...]:
        return self._incoming_material_state.pending(topic_id)

    def delete_pending_incoming_materials(self, topic_id: int, material_ids: Sequence[str]) -> int:
        return self._incoming_material_state.delete_pending(topic_id, material_ids)

    def stored_incoming_materials_for_terminal_jobs(
        self, topic_id: int
    ) -> tuple[IncomingMaterialRecord, ...]:
        return self._incoming_material_state.stored_for_terminal_jobs(topic_id)
    ...
    def mark_incoming_materials_discarded(
        ...
        *,
        ...
        return self._incoming_material_state.mark_discarded(
            material_ids,
            code=code,
            detail=detail,
        )
    ...
    def commit_provider_result(
        ...
        *,
        ...
        with self._immediate_transaction():
            ...
            self._incoming_material_state.mark_job_consumed(job_id, timestamp=timestamp)
            self._connection.execute(
    ...
    def _next_generation(self, topic_id: int, agent_id: str) -> int:
        return self._sessions_state._next_generation(topic_id, agent_id)
    ...
    def _insert_session(
        ...
        return self._sessions_state._insert_session(
            topic_id,
            agent_id,
            model,
            effort,
    ...
    def _require_control_snapshot(self, topic_id: int, expected_session_id: str | None) -> None:
        self._sessions_state._require_control_snapshot(topic_id, expected_session_id)
    ...
    def ensure_satellite(
        ...
        return self._sessions_state.ensure_satellite(topic_id, agent_id, model, effort)

    def new_active_session(
        self, topic_id: int, *, expected_session_id: str | None = None
    ) -> SessionRecord:
        return self._sessions_state.new_active_session(
            topic_id,
            expected_session_id=expected_session_id,
        )
