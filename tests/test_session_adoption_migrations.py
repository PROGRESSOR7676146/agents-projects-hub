...
class SessionAdoptionMigrationTests(unittest.TestCase):
        ...
        for fail in (False, True):
            with self.subTest(fail=fail), tempfile.TemporaryDirectory() as directory:
                ...
                if fail:
                    ...
                    self.assertEqual((result.previous_version, result.current_version), (24, 29))
                ...
                with closing(sqlite3.connect(path)) as connection:
                    self.assertEqual(
                        connection.execute("PRAGMA user_version").fetchone()[0], 24 if fail else 29
                    )
                    self.assertEqual(
    ...
    def test_distinct_rollback_fixture_retains_policy_and_v24_refuses_origins(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            descriptors = []
            for index, maximum in enumerate((29, 29, 24)):
                wheel = _wheel(
                ...
                if maximum == 24:
                    ...
                    continue
                self.assertEqual(_run_artifact_migration(extracted, state_path)["state_schema"], 29)
                script = """
import sys
from pathlib import Path
