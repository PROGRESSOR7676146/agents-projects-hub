...
class IngressAcceptanceTests(unittest.TestCase):
    ...
    def test_dynamic_project_sqlite_fault_retains_offset_and_retries_once(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            harness = FaultMatrixHarness(Path(directory))
            harness.config = replace(harness.config, projects=())
            service = harness.controller()
            try:
                with service.state._immediate_transaction():
                    service.state._connection.execute(
                        """INSERT INTO project_onboarding_workflows
                           ...
                           VALUES ('dynamic-example',42,'completed','2099','now','now')"""
                    )
                    service.state._connection.execute(
                        """INSERT INTO project_group_bindings
                           (project_id,telegram_chat_id,canonical_root,workflow_id,created_at)
                           VALUES ('example-project',?,?,'dynamic-example','now')""",
                        (harness.chat_id, str(harness.registry.projects[0].root)),
                    )
                message = harness.update(141, 841, "@example_opencode_bot fictional task")
                with patch.object(
                    service.state,
                    "ensure_satellite",
                    side_effect=sqlite3.OperationalError("database is locked"),
                ):
                    self.run_batches(service, [[message]])
                self.assertIsNone(service.state.get_bot_offset("hub"))
                self.run_batches(service, [[message], [message]])
                topic = service.state.find_topic(harness.chat_id, 841)
                assert topic is not None
                self.assertEqual(len(service.state.provider_jobs_for_topic(topic.topic_id)), 1)
                self.assertEqual(service.state.get_bot_offset("hub"), 142)
            finally:
                service.close()
