...
for current reusable behavior and the next milestone.

## Repository layout

```text
config/       publishable configuration examples (real config is ignored)
docs/         product, status, architecture, decisions, risks, operations, and tests
...
A private Hermes Telegram chat can serve as the administrative and recovery
...
- Complete owner-driven acceptance for provider/model selection and natural
  quota transitions in each deployment.
- Add automatic Antigravity account rotation only when its provider exposes a
  stable supported headless account-pool interface.
